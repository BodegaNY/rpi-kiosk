...
5. **Optional physical button** — momentary switch to a **BCM** GPIO and **GND** (pull-up inside the Pi). **Short tap(s):** after a quiet gap (~0.45 s), each tap in a 1- or 2-tap sequence advances **next view** once per tap (same order as auto-rotation). **Long hold (~1 s)** then release → **Dakboard**. **Three quick taps** → **toggle auto-rotate** (same as control panel). Constants: `GPIO_LONG_PRESS_SEC`, `GPIO_MULTI_TAP_GAP_SEC` at top of `kiosk-controller.py`. Set `gpio_button_bcm` in `~/.kiosk-config.json` (e.g. `17`) or `KIOSK_GPIO_BUTTON_BCM=17`. Requires `sudo apt install python3-rpi.gpio`. **Wiring:** GPIO pin + **GND** only (not 3.3 V / 5 V).
